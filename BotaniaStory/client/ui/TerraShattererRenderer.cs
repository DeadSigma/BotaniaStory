...
{
    public class TerraShattererRenderer : IRenderer
    {
        private ICoreClientAPI capi;
        private LoadedTexture bgTex;
        private LoadedTexture fgTex;

        // Кэш для наших цифр, чтобы не убить FPS
        ...
        public TerraShattererRenderer(ICoreClientAPI capi)
        {
            this.capi = capi;
            ...
            // Настраиваем ванильный шрифт (Белый цвет, размер 14, черная обводка)
        ...
        private LoadedTexture GetOrCreateLevelTexture(int level)
        {
            if (!levelTextures.TryGetValue(level, out LoadedTexture tex))
            {
                ...
                // ИСПРАВЛЕНИЕ: Метод сам создает и возвращает готовую текстуру!
        ...
        public void OnRenderFrame(float deltaTime, EnumRenderStage stage)
        {
            ...
            if (!targetSlot.Itemstack.Collectible.Code.Path.Contains("terrashatterer")) return;
